...
void led_init(void)
{
    ...
    GPIOB->MODER &= ~(3 << 28); // effacer les bits 28 et 29
    GPIOB->MODER |= (1 << 28);  // activer la sortie sur le port PB14

    GPIOC->MODER &= ~(3 << 18); // effacer les bits 18 et 19
...
void led_g_on(void)
{
    ...
    GPIOB->BSRR = (1 << 14); // mettre le bit 14 à 1 dans BSRR
}

void led_g_off(void)
{
    // réinitialiser PB14 à 0 pour éteindre la LED
    GPIOB->BSRR = (1 << (14 + 16)); // mettre le bit 14 à 1 dans la partie haute du registre BSRR (pour réinitialiser)
}

void led_j_on_b_off(void)
{
    // Quand PC9 est en sortie à l'état haut
    GPIOC->BSRR = (1 << 9); // mettre le bit 9 à 1 dans BSRR
}

void led_b_on_j_off(void)
{
    // Quand PC9 est en sortie à l'état bas
    GPIOC->BSRR = (1 << (9 + 16)); // mettre le bit 9 à 0 dans la partie haute du registre BSRR (pour réinitialiser)
}

void led_b_j_off(void)
{
    // Quand PC9 est en entre, LED3 et LED4 sont eteintes.
    GPIOC->MODER &= ~(3 << 18); // activer la mode entree sur le port PC9 (effacer les bits 18 et 19)
}
